...
OLD: Client for persisting chat message history in a Postgres database.
OLD: This client provides support for both sync and async via psycopg 3.

Made from: langchain_postgres library
...
def _create_table_and_index(table_name: str) -> List[sql.Composed]:
    """Make a SQL query to create a table.
        WILL NOT BE USED because the LLM user doesn't have permission to create tables in the DB.
    """
...
class PostgresChatMessageHistory(BaseChatMessageHistory):
    def __init__(
        ...
        *,
        ...
        # Set search path to app_ai schema
        if self._connection:
            self._connection.execute(f"SET search_path TO app_ai;")
        if self._aconnection:
            self._aconnection.execute(f"SET search_path TO app_ai;")
